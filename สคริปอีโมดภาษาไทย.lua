...
local Settings = {}
Settings["หยุดอีโมตเมื่อเดิน"] = true
Settings["เฟดเข้า"]     = 0.1
Settings["เฟดออก"]    = 0.1
Settings["น้ำหนัก"]      = 1
Settings["ความเร็ว"]       = 1
Settings["อนุญาตให้ล่องหน"]    = true
Settings["ตำแหน่งเวลา"] = 0
Settings["หยุดนิ่งเมื่อจบ"] = false
Settings["เล่นซ้ำ"] = true
Settings["หยุดแอนิเมชันอื่นเมื่อเล่น"] = true
...
local function LoadTrack(id)
    if CurrentTrack then 
        CurrentTrack:Stop(Settings["เฟดออก"]) 
    end
    local animId
    ...
    local weight = Settings["น้ำหนัก"]
    if weight == 0 then weight = 0.001 end
    if Settings["หยุดแอนิเมชันอื่นเมื่อเล่น"] then
    ...
    newTrack:Play(Settings["เฟดเข้า"], weight, Settings["ความเร็ว"])
    CurrentTrack = newTrack 
    CurrentTrack.TimePosition = math.clamp(Settings["ตำแหน่งเวลา"], 0, 1) * (CurrentTrack.Length or 1)
    CurrentTrack.Priority = Enum.AnimationPriority.Action4
    CurrentTrack.Looped = Settings["เล่นซ้ำ"]
    return newTrack
end
...
if Settings["เล่นซ้ำ"] and CurrentTrack and CurrentTrack.IsPlaying then
    CurrentTrack.Looped = Settings["เล่นซ้ำ"]
end

if character:FindFirstChild("HumanoidRootPart") then
    local root = character.HumanoidRootPart
    if Settings["หยุดอีโมตเมื่อเดิน"] and CurrentTrack and CurrentTrack.IsPlaying then
...
sortBtn.Text = "เรียงลำดับ: ความเกี่ยวข้อง"
...
savedSearch.PlaceholderText = "ค้นหาที่บันทึกไว้..."
...
savedEmptyLabel.Text = "ขอโทษด้วย กำลังเปลี่ยนไฟล์บันทึกอีกแล้ว 😅"
...
local function createSlider(name, min, max, default)
    ...
    local function applyValue(value)
        ...
        if CurrentTrack and CurrentTrack.IsPlaying then
            if name == "ความเร็ว" then
                CurrentTrack:AdjustSpeed(Settings["ความเร็ว"])
            elseif name == "น้ำหนัก" then
                local weight = Settings["น้ำหนัก"]
                if weight == 0 then weight = 0.001 end
                CurrentTrack:AdjustWeight(weight)
            elseif name == "ตำแหน่งเวลา" then
...
local resetButton = createButton("รีเซ็ตค่า", function() end)
createToggle("หยุดอีโมตเมื่อเดิน")
createToggle("เล่นซ้ำ")
createSlider("ความเร็ว", 0, 5, Settings["ความเร็ว"])
createSlider("ตำแหน่งเวลา", 0, 1, Settings["ตำแหน่งเวลา"])
createSlider("น้ำหนัก", 0, 1, Settings["น้ำหนัก"])
createSlider("เฟดเข้า", 0, 2, Settings["เฟดเข้า"])
createSlider("เฟดออก", 0, 2, Settings["เฟดออก"])
createToggle("อนุญาตให้ล่องหน")
createToggle("หยุดแอนิเมชันอื่นเมื่อเล่น")

resetButton.MouseButton1Click:Connect(function()
    Settings:EditToggle("หยุดอีโมตเมื่อเดิน", true)
    Settings:EditToggle("หยุดแอนิเมชันอื่นเมื่อเล่น", true)
    Settings:EditSlider("เฟดเข้า", 0.1)
    Settings:EditSlider("เฟดออก", 0.1)
    Settings:EditSlider("น้ำหนัก", 1)
    Settings:EditSlider("ความเร็ว", 1)
    Settings:EditToggle("อนุญาตให้ล่องหน", true)
    Settings:EditSlider("ตำแหน่งเวลา", 0)
    Settings:EditToggle("หยุดนิ่งเมื่อจบ", false)
    Settings:EditToggle("เล่นซ้ำ", true)
end)

local originalCollisionStates = {}
local lastFixClipState = Settings["อนุญาตให้ล่องหน"]
...
local function disableCollisionsExceptRootPart()
    if not Settings["อนุญาตให้ล่องหน"] then
        return
    end
    for _, part in ipairs(character:GetDescendants()) do
        if part:IsA("BasePart") and part ~= character.PrimaryPart then
...
connection = RunService.Stepped:Connect(function()
    if character and character.Parent then
        local currentFixClip = Settings["อนุญาตให้ล่องหน"]
...
player.CharacterAdded:Connect(function(newCharacter)
    ...
    humanoid = newCharacter:WaitForChild("Humanoid")
    saveCollisionStates()
    lastFixClipState = Settings["อนุญาตให้ล่องหน"]
    if connection then
        connection:Disconnect()
    end
    connection = RunService.Stepped:Connect(function()
...
local sortModes = {
    {Enum.CatalogSortType.Relevance, "ความเกี่ยวข้อง"},
    {Enum.CatalogSortType.PriceHighToLow, "ราคา สูง→ต่ำ"},
    {Enum.CatalogSortType.PriceLowToHigh, "ราคา ต่ำ→สูง"},
    {Enum.CatalogSortType.MostFavorited, "ยอดนิยม"},
    {Enum.CatalogSortType.RecentlyCreated, "สร้างล่าสุด"},
    {Enum.CatalogSortType.Bestselling, "ขายดี"},
}
...
local function createCard(item)
    ...
    name.Text = item.Name or "ไม่ทราบชื่อ"
...
emptyLabel.Text = "ไม่มีอะไรตลกที่นี่นะ :3 (ยกเว้นฉัน)"
...
local function showPage(pages)
    ...
    pageBox.Text = "หน้า " .. tostring(currentPageNumber) .. " / ใส่หน้า"
    updateNavVisibility()
...
sortBtn.MouseButton1Click:Connect(function()
    ...
    sortBtn.Text = "เรียงลำดับ: " .. sortModes[currentSortIndex][2]
...
pageBox.FocusLost:Connect(function(enterPressed)
    ...
    if not ok or not pages then
        pageNotif.Text = "โหลดหน้าไม่สำเร็จ"
        pageNotif.Visible = true
        task.delay(2, function() if pageNotif then pageNotif.Visible = false end end)
        pageBox.Text = "หน้า " .. tostring(currentPageNumber)
        return
